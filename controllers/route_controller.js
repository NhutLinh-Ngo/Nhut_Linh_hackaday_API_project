...
const fetchFunc = require('./fetch_functions');

exports.projectHomePage = async (req, res) => {
...
exports.loadNewPaginatedData = async (req, res) => {
	const page = req.query.page ? parseInt(req.query.page) : 1;
	const sortby = req.query.sortby;
...
exports.projectDetailsPage = async (req, res) => {
	...
	let project = await fetchFunc.getProjectInfoBy(projectId);
	...
	if (project.project == 0) {
		...
		if (project.details) {
			project.details_info = await fetchFunc
				.getProjectInfoBy(projectId, 'details')
		...
		if (project.images) {
			project.imagesArray = await fetchFunc
				.getProjectInfoBy(projectId, 'images')
				.then((res) => res.images);
		}
		...
		if (project.components) {
			project.components_info = await fetchFunc
				.getProjectInfoBy(projectId, 'components')
		...
		if (project.logs) {
			project.logs_info = await fetchFunc
				.getProjectInfoBy(projectId, 'logs')
				.then((res) => res.logs);
		}
		...
		if (project.instruction) {
			project.instruction_info = await fetchFunc
				.getProjectInfoBy(projectId, 'instructions')
